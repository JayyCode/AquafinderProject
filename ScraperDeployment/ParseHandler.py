...
class DBT:
        ...
        try:
            with open(log_file, "r", encoding="utf-8") as file:
                _, ext = os.path.splitext(
                    log_file
                )  # separates extension for file check
                ...
                if ext in extensions:
                    for element in data_set:
                        ...
                        if not temp_set:
                            print(
                                "---HAND-OFF NOT ACCEPTED: DATA NOT PULLED FROM REQUEST---"
                            )
                    return temp_set
                # baller brown nigga
                    ...
                    for element in data_set:  # list of json nodes
                        string_json = json.dumps(
                            element
                        )  # returns string of json node (key/value pairs)
                        parsedJson = json.loads(
                            string_json
                        )  # returns python dictionary of key/value pairs
                        content = parsedJson["results"][0][
                            "content"
                        ]  # accesses content in nested dictionary
                        temp_set.append(content)
                        if not temp_set:
                            print(
                                "---HAND-OFF NOT ACCEPTED: DATA NOT PULLED FROM JSON---"
                            )
                    return temp_set
            ...
            print(
                "---INVALID PERMISSIONS ERROR: THE FILES PERMISSIONS WERE CHANGED IN THE OS DURING HAND-OFF---"
            )
        except OSError as e:
            print(f"OS error: {e}")
    ...
    def data_query(self, temp_data, seller: str, links):
        ...
        while True:
            ...
            title_query = input(
                "Enter class name for product Title (Found within HTML code of any product: "
            )
        ...
        desc_query = input(
            "Enter class name for product Description (Found within HTML code of any product: "
        )
