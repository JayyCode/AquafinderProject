...
import time
import requests
from requests.exceptions import RequestException  # for error handling in the first try/catch block
...
class scaperAPI:
    ...
    def initScrape(self, url, nsURL=None, filter_key=None):
        ...
        try:
            ...
            if response.status_code == 200 or response.status_code == 202:
                ...
                xpath_dir = '//ns:loc[contains(text(), "{}")]/text()'.format(
                    filter_key) if filter_key else '//ns:loc/text()'
                ...
                hyperlinks = tree.xpath(xpath_dir, namespaces=nsmap)  # returns a list for the variable

                # the loop in the brackets is the nested if statement. read it backwards
                filtered_urls = [link for link in hyperlinks if filter_key in link] if filter_key else hyperlinks # Filter URLs based on the specified keyword
            ...
            # ---------------Response Content Check & Print ------------------------------------------------------------
                ...
                print('No valid response from server.. retreived code: ' + str(response.status_code))
    ...
    def taskRoute(self,url,nsURL=None,filter_key=None ):
        ...
        domain_parts = parsed_url.netloc.split('.')  # Split the domain name into parts based off periods
        ...
        if ans == 'Y' or ans == 'y':
            links = self.initScrape(url, nsURL, filter_key) # (returns link tree header)
            ...
            for link in links:
                    ...
                    response = requests.post(url, json=payload, auth=('U0000142550','PW107283e45d5223036a149c85891e200ca'))
